...
```bash
# 在 mini-claude-code 仓库根目录执行
mvn -q clean package -DskipTests
```

产物：`target/mini-claude-code-1.0-SNAPSHOT.jar`，所有用例都基于这个 jar 启动。
...
- Banner 显示简洁的 `Mini Claude Code v16.1.0` 开屏，不带右侧盒线边框
...
- ReAct 请求发出后，输入区上方出现动态 `Thinking...` 面板；如果模型返回 reasoning delta，面板下方显示灰色 `> ...`
  引用行，最终回复开始时面板被清理
...
**注意**：Agent 必须在 **同一轮**返回 3 个 tool call 才会聚合成一个折叠块。如果 Agent 拆成 3 轮（每轮 1 个），会出现 3
个独立折叠块——这是 LLM 的决策，不是 bug。
...
**预期**：LLM 流式输出过程中，`┌─ code: markdown` 出现后，body 部分 **不会持续刷屏**； 等代码块结束时，原 header 被覆盖为：
...
```bash
MINI_CLAUDE_CODE_RENDERER=lanterna java -jar target/mini-claude-code-1.0-SNAPSHOT.jar
```
...
## 11. 形态切换 — 兼容旧 MINI_CLAUDE_CODE_TUI（用例 10）
...
- stderr 出现 `⚠️ MINI_CLAUDE_CODE_TUI=true 已废弃，请改用 MINI_CLAUDE_CODE_RENDERER=lanterna`
...
```bash
MINI_CLAUDE_CODE_RENDERER=plain java -jar target/mini-claude-code-1.0-SNAPSHOT.jar
```
...
```bash
NO_COLOR=1 java -jar target/mini-claude-code-1.0-SNAPSHOT.jar
```
...
```bash
MINI_CLAUDE_CODE_RENDERER=weird java -jar target/mini-claude-code-1.0-SNAPSHOT.jar
```

**预期**：stderr `⚠️ 未识别的 MINI_CLAUDE_CODE_RENDERER='weird'，回退到 inline`，正常进 inline 模式。
...
```bash
TERM=dumb java -jar target/mini-claude-code-1.0-SNAPSHOT.jar
```
...
```
*
 Mini Claude Code  idle  glm-5.1  ctx 0/200.0k  HITL OFF  MCP 4/4  Skill 2/2
...
```
把 ROADMAP.md 第一行改成 "# Mini Claude Code 路线图（v16.1 测试）"
```
...
📝 ROADMAP.md
@@ -1,N +1,N @@
- # Mini Claude Code ROADMAP（原标题）
+ # Mini Claude Code 路线图（v16.1 测试）
  ...（上下文行）
```
...
**预期**：如果 before 内容读不出（二进制 / 编码错），diff 会按"新建文件"形态展示——这是 ToolRegistry.write_file lambda 里
`before == null` 的兜底分支。
...
│   [6] 配置文件: ~/.mini-claude-code/config.json (只读视图，编辑请用编辑器)
...
**预期**：弹出 Lanterna 模态选择列表（不是 inline 浮起 palette），选项内容相同。选中后 inline 模式下的提示文案应该出现在
CenterPane（对话流）中。

---

## 23. MINI_CLAUDE_CODE_NO_STATUSBAR 禁用状态栏

```bash
MINI_CLAUDE_CODE_NO_STATUSBAR=true java -jar target/mini-claude-code-1.0-SNAPSHOT.jar
```
...
## 28. /plan 工具调用折叠块与 web 摘要（394e2e1 / 40d2d65 回归）

验证 PlanExecuteAgent 注入渲染器后，`/plan` 的工具调用获得与 ReAct 相同的折叠块，
且 web 工具摘要行在 Plan 路径正常出现。

**启动**（同用例 1，需要已配置 web_search 可用的 provider）：
...
```
/plan 搜索一下 2026 年 Java 虚拟线程的最新进展，然后总结成三句话
```

**预期**：

- 计划生成、确认执行后，任务步骤中的工具调用以 **单行折叠态** 输出，而不是旧版永远展开的多行列表：

```
⏵ 联网搜索 1 次 (ctrl+o to expand)
```

- 按 **Ctrl+O** 展开可见工具名 + query 参数行，再按收起
- `web_search` 返回后立刻出现一行灰色摘要，位置在折叠块之后、回复正文之前：

```
  → 搜索 "2026 Java 虚拟线程" 返回 N 条结果
```

- 摘要行只有一行；失败时是 `→ 搜索失败: ...` 单行压缩
...
验证 SubAgent 路径的 web 摘要跟随 step 级缓冲流：多步并行时摘要不交错，
重放后摘要行位于所属步骤输出块内部。
...
```
/team 并行做三件事：搜索"Java 21"新特性、搜索"Go 1.24"新特性、搜索"Rust 2026"进展，各自总结一段
```

**预期**：

- 多步并行执行时终端不出现交错碎片；每个步骤的完整输出（含其工具折叠块 / 工具行与摘要行）按 step 顺序整块出现
- 含 web_search 的步骤，其输出块内部包含该步骤自己的摘要行：

```
[step_1] ...
  → 搜索 "Java 21 新特性" 返回 N 条结果
...
[step_2] ...
  → 搜索 "Go 1.24 新特性" 返回 N 条结果
```

- 不同步骤的摘要行不出现交叉（如 step_1 块里出现 step_2 的 query）
...
- 旧 Windows cmd / PuTTY 等不适合 ANSI 光标控制的终端：用 `MINI_CLAUDE_CODE_NO_STATUSBAR=true` 即可
