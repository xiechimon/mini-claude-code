...
- 默认数据库：`~/.mini-claude-code/tasks/tasks.db`
- 生命周期：
    - `enqueued`
    - `running`
    - `completed`
    - `failed`
    - `canceled`
- Worker Pool：默认 2 个后台 worker，可用 `MINI_CLAUDE_CODE_TASK_WORKERS` 或 `-Dmini-claude-code.task.workers` 覆盖
...
实现位于 `src/main/java/com/miniclaudecode/runtime/api/`，使用 JDK 内置 `HttpServer`，不引入 Spring / Javalin。

启动：

```bash
MINI_CLAUDE_CODE_RUNTIME_API_KEY=your_local_api_key \
...
- 仅监听 `127.0.0.1`
- 必须配置 `MINI_CLAUDE_CODE_RUNTIME_API_KEY` 或 `-Dmini-claude-code.runtime.api.key`
- 请求头支持：
    - `Authorization: Bearer <key>`
    - `X-Mini-Claude-Code-API-Key: <key>`
...
MINI_CLAUDE_CODE_RUNTIME_API_KEY=test java -jar target/mini-claude-code-1.0-SNAPSHOT.jar serve --http --port 0
```
