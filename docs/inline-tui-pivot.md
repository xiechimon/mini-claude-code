...
phase-16 选了 Lanterna 全屏窗口型 TUI（文件树 + 对话流 + 状态栏 + 输入栏）， 对照真正的目标体验——Claude Code、Qoder
CLI、Codex CLI、Gemini CLI、Aider——是 **走错了流派**。
...
`Read 3 files (ctrl+o to expand)` 这种行内可折叠工具块、行内 git diff、单字符 HITL 提示—— Lanterna 的 widget 体系做不出来（一旦
`startScreen()` 进 alternate buffer，所有内容都活在它的 buffer 里；widget
是固定盒子，没有"折叠展开导致下方消息向下推"的滚屏语义）。

**调整方向**：抽 `Renderer` 接口，inline 流式作为默认实现，Lanterna 作为另一个可切换的实现。 两套并存的代价是接口抽象层 +
双倍维护，但好处是 phase-16 的 Lanterna 工作量不浪费、给用户保留全屏窗口选择、教学价值更高。

---

## 2. 什么是 Inline 流式 TUI（精确定义）

不进 alternate screen buffer（不发 `\e[?1049h`），主屏直接输出， 靠 ANSI 控制序列做 **局部重绘**和 **底部固定区域**：
...
新建 `com.miniclaudecode.render` 包，把 Agent / HITL / Main 对终端的全部输出收口到一个接口：
...
public interface Renderer extends AutoCloseable {
    ...
    // ---- 配置面板 / 命令选择 ----
    void openConfigPanel(MiniClaudeCodeConfig config);
...
- `ToolBlock` 是 handle 模式：`startToolCall` 返回后 `appendResult` 异步填充——inline 实现里它对应 `FoldableBlock`
  ，Lanterna 实现里对应 `CenterPane.appendToolResult` 的延迟调用

### 4.2 包结构

```
com.miniclaudecode.render/                       ← 新增（接口层）
├── Renderer.java                        核心接口
...
└── RendererFactory.java                 根据 MINI_CLAUDE_CODE_RENDERER 创建实例

com.miniclaudecode.render.inline/                ← 新增（inline 实现）
...
com.miniclaudecode.render.plain/                 ← 新增（纯 println 降级）
└── PlainRenderer.java                   等价 phase-15 行为，无折叠、无状态栏

com.miniclaudecode.tui/                          ← 保留（Lanterna 实现）
...
`com.miniclaudecode.tui.highlight.CodeHighlighter` 两个实现都用，不迁移。

### 4.3 `RendererFactory` 选型

```java
public final class RendererFactory {
    public static Renderer create(Terminal terminal, MiniClaudeCodeConfig config) {
        String mode = resolveMode();  // env: MINI_CLAUDE_CODE_RENDERER
        return switch (mode) {
...
| 变量                                 | 含义                                                                       |
|--------------------------------------|----------------------------------------------------------------------------|
| 默认                                 | `inline`（流式 transcript）                                                |
| `MINI_CLAUDE_CODE_RENDERER=lanterna` | Lanterna 全屏 TUI                                                          |
| `MINI_CLAUDE_CODE_RENDERER=plain`    | 纯 println，无折叠、无状态栏                                               |
| `MINI_CLAUDE_CODE_RENDERER=inline`   | 显式声明 inline                                                            |
| `NO_COLOR=1`                         | 已有：禁用 ANSI 颜色，对三种模式都生效                                     |
| `MINI_CLAUDE_CODE_TUI=true`          | 兼容旧版：等价于 `MINI_CLAUDE_CODE_RENDERER=lanterna`，打 deprecation 提示 |

### 4.4 修改的现有文件

| 文件                                                    | 改动                                                                                                                                                                            |
|---------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `com.miniclaudecode.cli.Main`                           | 启动时 `Renderer renderer = RendererFactory.create(...)`；删除 `TuiBootstrap.shouldUseTui` 二分支判断；CLI 主循环全部经 `renderer` 调用，不再直接 `System.out.println`          |
| `com.miniclaudecode.agent.Agent$StreamRenderer`         | 持有 `Renderer`；`onReasoningDelta`/`onContentDelta` 走 `renderer.appendReasoningChunk/appendContentChunk`；`appendToolCall` 走 `renderer.startToolCall(...).appendResult(...)` |
| `com.miniclaudecode.agent.PlanExecuteAgent`、`SubAgent` | 同上                                                                                                                                                                            |
| `com.miniclaudecode.hitl.TerminalHitlHandler`           | 改名 `RendererHitlHandler`，持有 `Renderer`；`requestApproval` 直接调 `renderer.promptApproval(req)`，不区分 CLI / TUI                                                          |
| `com.miniclaudecode.hitl.SwitchableHitlHandler`         | 简化：只剩开关逻辑（enable/disable），delegate 永远是同一个 `RendererHitlHandler`                                                                                               |
| `com.miniclaudecode.hitl.TuiHitlHandler`                | 删除（被 `LanternaRenderer.promptApproval` 取代）                                                                                                                               |
| `com.miniclaudecode.tui.TuiBootstrap`                   | 删除 `shouldUseTui()` / `launch()`；保留为空类或直接删除（`LanternaRenderer` 自己负责启动 Lanterna 主循环）                                                                     |
| `com.miniclaudecode.tui.TuiSessionController`           | 删除（职责由 `Main` + `LanternaRenderer` 承担）                                                                                                                                 |
| `com.miniclaudecode.util.AnsiStyle`                     | 扩充 cursor save/restore、line clear、scroll region、reverse 等序列；底层常量挪到 `com.miniclaudecode.render.inline.AnsiSeq`                                                    |
...
`LanternaRenderer` 是适配器，不重写 Lanterna 已有的 widget 逻辑——保留 `CenterPane` / `StatusPane` / `TuiHitlHandler`，只是
`Renderer` 方法到 widget 方法的薄封装。
...
**全部保留**：`com.miniclaudecode.tui.*` 在新架构下作为 Lanterna 实现存在，不删代码。
...
- `docs/phase-16-tui-productization.md`：在文档顶部加注 "本文档形态选型已被 inline-tui-pivot.md 修正：phase-16 工作以
  Lanterna 实现的形式保留，但默认形态切换为 inline 流式"
- `AGENTS.md`：第 16 期描述更新为"双形态可切换：默认 inline 流式，可切换 Lanterna 全屏"
- `README.md`：新增"渲染器形态切换"段，说明 `MINI_CLAUDE_CODE_RENDERER` 三档
- `ROADMAP.md`：第 16 期标 ✅，下一步 phase-17 为 LSP 诊断注入，图片输入后移 phase-21
- `.env.example`：把 `MINI_CLAUDE_CODE_TUI=true` 标 deprecated，新增 `MINI_CLAUDE_CODE_RENDERER=inline|lanterna|plain`
...
- 新建 `com.miniclaudecode.render` 包：`Renderer` / `ToolBlock` / `StatusInfo` / `ApprovalRequest` / `ApprovalDecision` /
  `PaletteItem` / `PaletteResult`
- `PlainRenderer`：纯 `System.out.println`，状态栏 / 折叠 / palette 都退化为简单打印
- `RendererFactory.create()` 实现，环境变量解析
- `Main.java` 接入：删除 `TuiBootstrap.shouldUseTui` 分支，改 `Renderer renderer = RendererFactory.create(...)`，主循环对话输出全部经
  `renderer`
...
- `com.miniclaudecode.render.inline` 包落地
...
- `InlineRenderer.appendReasoningChunk` / `appendContentChunk` / `finishStream` 实现（直写 stdout，复用
...
- Banner 微调："Mini Claude Code v16.1.0"（小版本号区分形态修正）

---

## 8. 端到端手测清单（Day 6 必跑，inline / lanterna 双形态）

每条用例在 `MINI_CLAUDE_CODE_RENDERER=inline`（默认）和 `MINI_CLAUDE_CODE_RENDERER=lanterna` 各跑一遍：

1. ✅ 启动 Mini Claude Code，状态栏/状态面板显示模型 + token
...
8. ✅ `MINI_CLAUDE_CODE_RENDERER=plain`：纯 println，无折叠、无状态栏
9. ✅ 终端 resize：inline 状态栏重新定位 + 已折叠块标 frozen；lanterna widget 自适应
10. ✅ Vim 模式 `MINI_CLAUDE_CODE_VI=true`：输入框支持 hjkl 移动
11. ✅ 200 行代码粘贴：格式保留，不触发提交
12. ✅ Lanterna 启动失败模拟：自动回退 inline，打日志提示
13. ✅ `MINI_CLAUDE_CODE_TUI=true`（deprecated 兼容）：等价 lanterna，提示 deprecation
...
| 默认形态                       | **inline**                                                          |
| 切换机制                       | **启动时**通过 `MINI_CLAUDE_CODE_RENDERER` 选择，不支持运行时热切换 |
| 接口位置                       | `com.miniclaudecode.render` 包，与 inline / lanterna 实现解耦       |
...
| 旧 `MINI_CLAUDE_CODE_TUI=true` | 兼容映射到 `MINI_CLAUDE_CODE_RENDERER=lanterna`，打 deprecation     |
...
- [ ] `com.miniclaudecode.render` 接口包落地（`Renderer` + 全部数据载体）
- [ ] `com.miniclaudecode.render.inline` 落地，`InlineRenderer` 通过所有单测
- [ ] `com.miniclaudecode.render.plain.PlainRenderer` 落地
- [ ] `com.miniclaudecode.tui.LanternaRenderer` 落地，`TuiBootstrap` / `TuiSessionController` / `TuiHitlHandler` 删除
- [ ] `RendererFactory` 三档切换工作正常，`MINI_CLAUDE_CODE_TUI=true` 兼容映射
