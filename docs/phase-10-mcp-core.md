...
> 3. 现有的 `src/main/java/com/miniclaudecode/tool/ToolRegistry.java`、
     `src/main/java/com/miniclaudecode/hitl/HitlToolRegistry.java`、
     `src/main/java/com/miniclaudecode/policy/AuditLog.java` —— 你要集成进这三处
>
> **执行原则**：
...
让 Mini Claude Code 接入 MCP 生态：stdio 子进程 server 与 Streamable HTTP 远程 server 都能用，工具自动注册到
`ToolRegistry`，与 HITL / AuditLog 协同。 **默认开启**。

最终交付：

- `com.miniclaudecode.mcp` 包（4 个子包，约 15 个类、~1500 行代码）
...
```
src/main/java/com/miniclaudecode/mcp/
...
测试 mirror 这个结构放到 `src/test/java/com/miniclaudecode/mcp/`。
...
跟 Claude Code 对齐，完整 `mcp__` 前缀让 LLM 一眼区分本地工具 vs MCP 工具。同 server 内同名工具直接报错，要求用户在配置里
alias。
...
- 标准错误码：`-32700/parse` `-32600/invalid_request` `-32601/method_not_found` `-32602/invalid_params`
...
- 用户级 `~/.mini-claude-code/mcp.json` + 项目级 `.mini-claude-code/mcp.json`， **项目级覆盖用户级**（按 server 名 merge）
...
- server 崩溃（stdout 流断开）：第一版 **不自动重启**，标 `status: ERROR`，工具从 ToolRegistry 移除；用户
  `/mcp restart <name>` 手动恢复
- HTTP server 连续 3 次请求失败：标 ERROR

### 3.9 HITL 默认审批

`com.miniclaudecode.hitl.ApprovalPolicy.requiresApproval` 改为：
...
`com.miniclaudecode.tool.ToolRegistry.AUDIT_TOOLS` 扩展为「危险工具集 + 任何 `mcp__` 前缀工具」。审计字段保持不变；`tool`
字段就是 `mcp__filesystem__read_file`，过滤时按前缀。
...
- `src/main/java/com/miniclaudecode/mcp/` 整个包
- `src/test/java/com/miniclaudecode/mcp/` 测试

**修改**：

- `src/main/java/com/miniclaudecode/tool/ToolRegistry.java`：新增 `registerMcpTool(McpToolDescriptor)` /
  `unregisterMcpTool(String toolName)`，`executeTool` 路由到 `McpToolBridge`；`AUDIT_TOOLS` 判断扩展为含 `mcp__` 前缀
- `src/main/java/com/miniclaudecode/hitl/ApprovalPolicy.java`：`requiresApproval` 加 `mcp__` 前缀判断
- `src/main/java/com/miniclaudecode/hitl/TerminalHitlHandler.java`：弹窗加 server 信息行
- `src/main/java/com/miniclaudecode/policy/AuditLog.java`：新增 args 脱敏（regex 替换 token / Bearer / authorization）
- `src/main/java/com/miniclaudecode/cli/Main.java`：启动时 `McpServerManager.startAll`；shutdown hook；CLI 命令分支；启动
  banner 后打印 MCP server 状态
- `src/main/java/com/miniclaudecode/cli/CliCommandParser.java`：5 个新 `MCP_*` 命令枚举与 parse 分支
- `src/main/java/com/miniclaudecode/agent/Agent.java`：系统提示词加 MCP 工具说明
- `src/main/java/com/miniclaudecode/agent/PlanExecuteAgent.java`：同上
- `src/main/java/com/miniclaudecode/agent/SubAgent.java`：同上
- `pom.xml`：评估下来不需要新依赖（Jackson + OkHttp 已有），如果中途发现需要请回到上游确认

**联动文档**（按 AGENTS.md 5.x 硬规则）：

- `AGENTS.md`：项目快照里把第 10 期标为已完成；新增「11. MCP 协议接入」段；硬规则加一条「改 MCP 要联动」；版本号根据是否升 v10
  决定（见 §10）
...
**不**实现 `/mcp add` / `/mcp remove`：配置编辑通过文件 + 重启 Mini Claude Code。
...
2. **GLM/DeepSeek 对 JSON Schema 的容忍度差**：MCP server 返回的 schema 可能用了 `$ref` / 深度 `anyOf` /
   `additionalProperties` 等，模型直接报错。schema 清洗（§3.5）必须做，且要写测试覆盖至少 3 种坏 schema。
...
- 环境变量缺失 → **报错**，不静默保留
- server 启动失败 → **不阻塞** Mini Claude Code 启动，标 ERROR 跳过
- 工具描述截断长度：1000 字符
- MCP 工具调用日志：写到 `~/.mini-claude-code/logs/mcp/<server>.log`（按天滚动，复用 logback 风格但用单独 logger）
...
- **集成测**（可选，CI 默认不跑）：标记 `@EnabledIfEnvironmentVariable("MCP_INTEGRATION_TEST", "true")`，跑真实 filesystem /
  fetch server
...
完成第 10 期后，把 `Main.java` 的 `VERSION` 升到 `10.0.0`，banner 标语改成 `MCP-Enabled Agent CLI`（或其他你认为更好的，但要在
commit 中说明理由）。
...
- [ ] §4 列出的所有现有文件都按要求修改
- [ ] §3.11 args 脱敏已实现并有测试
- [ ] 单测覆盖：JsonRpcClient ≥ 8 用例、StdioTransport ≥ 5 用例、StreamableHttpTransport ≥ 6 用例、McpClient ≥ 5
  用例、McpConfigLoader ≥ 4 用例、McpServerManager ≥ 4 用例、McpToolBridge ≥ 3 用例、ApprovalPolicyTest 与 AuditLogTest
  各追加 ≥ 1 用例
- [ ] `mvn test` 全绿（包含原有 261 用例）
- [ ] §8 手测清单 10 条全部跑过且 OK
- [ ] §4 所有文档联动完成
- [ ] Banner 升 v10.0.0
- [ ] commit message 说明 MCP 核心改动和测试证据
...
| 配置文件位置                       | `~/.mini-claude-code/mcp.json`（用户级）+ `.mini-claude-code/mcp.json`（项目级，可入 git） |
