...
> 当前状态：MVP 已落地。目标是把 ReAct / Plan / Team / Planner 的 system prompt 从 Java 源码中抽离为 Markdown
> 资源，并支持用户级、项目级覆盖。
...
```text
src/main/java/com/miniclaudecode/prompt/
...
2. 用户级覆盖：`~/.mini-claude-code/prompts/...`
3. 项目级覆盖：`.mini-claude-code/prompts/...`

例如：

```text
~/.mini-claude-code/prompts/base.md
~/.mini-claude-code/prompts/modes/agent.md
.mini-claude-code/prompts/modes/team-worker.md
```
...
- [x] 支持用户级覆盖 `~/.mini-claude-code/prompts/...`
- [x] 支持项目级覆盖 `.mini-claude-code/prompts/...`
- [x] 校验 `## Language`
- [x] ReAct 接入
