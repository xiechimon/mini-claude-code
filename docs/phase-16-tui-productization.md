...
> 默认渲染器切换为 **inline 流式 TUI**；
> Lanterna 三栏 TUI 作为可切换形态保留，通过 `MINI_CLAUDE_CODE_RENDERER=lanterna` 启用。
...
> 4. `src/main/java/com/miniclaudecode/cli/Main.java`（当前 CLI 入口，TUI 将在此之上做可视化层）
> 5. `src/main/java/com/miniclaudecode/util/TerminalMarkdownRenderer.java`（现有 Markdown 渲染器，TUI 需复用或增强）
> 6. `src/main/java/com/miniclaudecode/agent/Agent.java`、`PlanExecuteAgent.java`、`SubAgent.java`（系统提示词构造路径）
>
> **核心原则**：本期 **不写新工具、不修改 Agent 核心逻辑**，只做 **可视化层**。TUI 是 Main.java 的"皮肤"，ReAct / Plan / Team
> 三条执行路径完全不动，只是把原来 JLine 行编辑器的输入输出升级成 Lanterna 面板布局。页面布局、样式定制、键盘交互是本期重点，LLM
> 协议和工具层保持稳定。
...
让 Mini Claude Code 从"纯 CLI"升级为"终端 GUI"（TUI），具备文件树浏览、代码高亮、对话历史可视化、配置管理等产品级体验，
**不牺牲任何现有 Agent 能力**。

**为什么做 TUI 而不是继续纯 CLI**：

- Mini Claude Code 当前已内置 9 个工具 + MCP 60+ 个工具，用户记忆成本高
- 文件树 / 代码高亮 / 对话历史回滚这些"展示型"需求，纯 CLI 手工渲染成本高且体验差
- 终端本身就是项目的主要交互界面，需要稳定呈现输入、流式输出、状态和审批
...
- **代码高亮显示**：对话中代码块自动语法高亮（支持 Java / Python / TypeScript / Bash / JSON / Markdown），`read_file` 结果在
  TUI 内内联展示
...
**不破坏现有命令行行为**：默认仍使用纯 CLI（JLine 行编辑器模式）。Lanterna 全屏 TUI 只在显式设置 `MINI_CLAUDE_CODE_TUI=true`
或 `-Dmini-claude-code.tui=true` 时启用；如果 `NO_TUI=true`、终端不可用或终端尺寸过小（`< 80×24`），仍降级为 CLI。
...
1. 未设置 `MINI_CLAUDE_CODE_TUI=true` 且未设置 `-Dmini-claude-code.tui=true` → 保持默认 CLI
2. `NO_TUI=true` → 降级
...
**高亮引擎**： **不使用第三方高亮库**（如 Pygments / highlight.js 的 Java 移植版），而是用 Mini Claude Code 自己第 4 期
`CodeChunker` 和 `CodeAnalyzer` 的能力做 **轻量级语法着色**。

实现路径：

1. 从 `CodeChunker` 拿代码块的 `language` 字段
2. 内置 **正则表达式词法着色器**（支持 Java / Python / TypeScript / Bash / JSON / Markdown / XML / YAML），基于关键字 /
   字符串 / 注释三类 token
3. 输出 ANSI 256 色字符串，直接交给 Lanterna `TextArea` 渲染

**为什么不用第三方库**：

- Mini Claude Code 已有 AST 解析能力（第 4 期 JavaParser），Java 代码着色可以复用
...
- 读：`MiniClaudeCodeConfig.load()` 已有
- 写：`MiniClaudeCodeConfig.save(config)` 新增，写入 `~/.mini-claude-code/config.json`
...
- **深色主题（默认）**：背景 `#1e1e2e`（Catppuccin Mocha 基调），文字 `#cdd6f4`，代码关键字 `#cba6f7`（紫），字符串 `#a6e3a1`
  （绿），注释 `#6c7086`（灰）
- **浅色主题**：背景 `#eff1f5`（Catppuccin Latte），文字 `#4c4f69`，代码关键字 `#8839ef`，字符串 `#40a02b`，注释 `#9ca0b0`

**主题持久化**：`~/.mini-claude-code/config.json` 的 `theme` 字段（`"dark"` / `"light"`）

**Lanterna 颜色映射**：用 `TextColor.ANSI.foreground(颜色编号)` 或 `TextColor.fromRGB(r, g, b)`，深色主题用 256 色或
TrueColor，浅色主题同样适配。
...
### 3.1 `~/.mini-claude-code/config.json` 新增字段
...
### 3.2 `~/.mini-claude-code/filetree-ignore.txt`（用户级忽略规则）
...
# ========== 第 16 期：TUI 产品化 ==========
# TUI 开关
# MINI_CLAUDE_CODE_TUI=true       # 显式启用 Lanterna 全屏 TUI；默认保持 CLI
# NO_TUI=true           # 强制 CLI，覆盖 MINI_CLAUDE_CODE_TUI=true
# MINI_CLAUDE_CODE_THEME=dark     # TUI 主题: dark / light（默认 dark）

# TUI 窗口尺寸（Lanterna 自适应，无需手动设置）
# MINI_CLAUDE_CODE_TUI_FILE_TREE_WIDTH=25%  # 文件树宽度（百分比或绝对列数）
# MINI_CLAUDE_CODE_TUI_FONT_SIZE=1          # 字体缩放（0=小, 1=中, 2=大）
```
...
### 4.1 新增类（`com.miniclaudecode.tui` 包）

| 文件                                                                           | 职责                                                                                                  |
|--------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------|
| `src/main/java/com/miniclaudecode/tui/TuiBootstrap.java`                       | TUI 入口，判断 `shouldUseTui()`，创建 `LanternaWindow` 和布局                                         |
| `src/main/java/com/miniclaudecode/tui/LanternaWindow.java`                     | 封装 Lanterna `Window`，统一处理主题 / 大小变化 / 输入分发                                            |
| `src/main/java/com/miniclaudecode/tui/pane/LeftPane.java`                      | 左侧文件树面板（`FileTreePane` 类，基于 Lanterna `TreeWidget` 或自定义 `ListWidget`）                 |
| `src/main/java/com/miniclaudecode/tui/pane/CenterPane.java`                    | 中央对话流面板（`ChatPane`，基于 Lanterna `TextArea`，支持 Markdown 渲染 + 代码高亮 + 消息分色）      |
| `src/main/java/com/miniclaudecode/tui/pane/RightPane.java`                     | 右侧状态栏面板（`StatusPane`，显示模型 / Token / 耗时 / 命令提示）                                    |
| `src/main/java/com/miniclaudecode/tui/pane/InputBar.java`                      | 底部输入栏（`InputBar`，基于 Lanterna `TextField`，处理 Enter / Tab / ↑↓ / Ctrl 快捷键）              |
| `src/main/java/com/miniclaudecode/tui/highlight/CodeHighlighter.java`          | 轻量级语法高亮器（基于 `CodeChunker` 语言识别 + 正则词法着色，输出 ANSI 256 色字符串）                |
| `src/main/java/com/miniclaudecode/tui/highlight/SyntaxTheme.java`              | 主题定义（深色 / 浅色），映射语言关键字 / 字符串 / 注释到 Lanterna `TextColor`                        |
| `src/main/java/com/miniclaudecode/tui/history/ConversationHistoryManager.java` | 对话历史管理（按 `ConversationSnapshot` 组织，支持保存 / 恢复 / 删除）                                |
| `src/main/java/com/miniclaudecode/tui/history/ConversationStore.java`          | 对话历史持久化（JSONL 格式，按天分文件 `~/.mini-claude-code/history/conversations-YYYY-MM-DD.jsonl`） |
| `src/main/java/com/miniclaudecode/tui/config/ConfigEditor.java`                | 配置管理面板（`/config` 命令触发，支持 API Key / 模型 / 上下文模式 / HITL / Skill 启用状态编辑）      |
| `src/main/java/com/miniclaudecode/tui/theme/ThemeManager.java`                 | 主题管理器（深色 / 浅色切换，持久化到 `config.json`）                                                 |
| `src/main/java/com/miniclaudecode/tui/theme/ColorPalette.java`                 | Catppuccin Mocha / Latte 调色板（背景 / 前景 / 关键字 / 字符串 / 注释等）                             |

### 4.2 修改类

| 文件                                                                  | 改动                                                                                                                                                                                 |
|-----------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `src/main/java/com/miniclaudecode/cli/Main.java`                      | TUI 入口判断（`shouldUseTui()`）；降级时保留现有 CLI；TUI 模式下用 `TuiBootstrap.launch()` 替换 JLine `LineReader` 主循环；Banner v16.0.0                                            |
| `src/main/java/com/miniclaudecode/cli/CliCommandParser.java`          | 新增 `CONFIG` 命令类型 + 子命令（`/config` 开配置面板）；TUI 模式下 `/clear` 清空对话流；`/exit` / `/quit` 关闭 TUI 窗口                                                             |
| `src/main/java/com/miniclaudecode/util/TerminalMarkdownRenderer.java` | 增强 Markdown 渲染器：支持代码块折叠 + 语法高亮输出（调用 `CodeHighlighter`）；保持 CLI 模式向后兼容                                                                                 |
| `src/main/java/com/miniclaudecode/config/MiniClaudeCodeConfig.java`   | 新增 `tui` 配置段（`fileTree` / `editor`）；新增 `ThemeManager` 读取 / 写入 `theme` 字段                                                                                             |
| `src/main/java/com/miniclaudecode/agent/Agent.java`                   | 流式输出时 TUI 模式改调用 `ChatPane.appendStreamingMessage()`（直接向面板写流式文本，不经过 `System.out`）；TUI 下的 HITL 弹窗调用 `TuiHitlDialog.show()` 替代 `TerminalHitlHandler` |
| `src/main/java/com/miniclaudecode/hitl/TerminalHitlHandler.java`      | 新增 `TuiHitlDialog` 内部类（Lanterna 模态框弹窗），保留 CLI 模式兼容；`requestApproval` 根据 `LanternaWindow.isTuiMode()` 分发到 TUI 或 CLI                                         |
| `src/main/java/com/miniclaudecode/agent/PlanExecuteAgent.java`        | 同上：流式输出和 HITL 弹窗根据模式分发                                                                                                                                               |
| `src/main/java/com/miniclaudecode/agent/SubAgent.java`                | 同上：Multi-Agent 流式输出分发                                                                                                                                                       |
...
**关键约束**：`ChatPane.append*` 方法必须是 **线程安全**的，因为 Agent 流式输出和工具执行结果是并行到达的。Lanterna
`TextArea` 本身不是线程安全的，需要用 `screen.getEventThread().postRunnable()` 或 `AtomicReference` 保证串行化。
...
**懒加载触发**：`TreeWidget` 的 `expand(int index)` 回调里 `Files.list(path)` 加载子节点，过滤掉 `filetree-ignore.txt`
规则匹配的条目。

**文件树更新**：`/clear` 命令不刷新文件树（项目结构不变）；需要手动 `Ctrl+R` 或
`ToolRegistry.executeTools(["read_file", "list_dir"])` 后触发刷新。
...
- `JavaLexer`：`\b(public|private|class|static|void|new|return|if|else|for|while)\b`（关键字）+ `"([^"\\]|\\.)*"`（字符串）+
  `//.*$`（行注释）+ `/\*[\s\S]*?\*/`（块注释）
- `PythonLexer`：`\b(def|class|import|from|if|elif|else|for|while|return|True|False|None)\b` + `'''[\s\S]*?'''` /
  `"""[\s\S]*?"""` + `#.*$`
...
1. `MiniClaudeCodeConfig` 初始化时读 `config.json` 的 `tui.theme` 字段
2. `ThemeManager` 维护当前主题（`"dark"` 默认）
3. `/config` 编辑主题后调用 `MiniClaudeCodeConfig.save(config)` 写回
4. 下次启动时读取
...
```bash
# 终端尺寸 ≥ 80×24，显式启用 TUI
MINI_CLAUDE_CODE_TUI=true java -jar target/mini-claude-code-1.0-SNAPSHOT.jar
```
...
```bash
MINI_CLAUDE_CODE_TUI=true NO_TUI=true java -jar target/mini-claude-code-1.0-SNAPSHOT.jar
```
...
3. 用 `↑` / `↓` 移动选中 `Agent.java`，按 `Enter` 插入 `@file://src/main/java/com/miniclaudecode/agent/Agent.java`
4. Agent 收到引用后调 `read_file` 并展示内容

### 7.3 代码高亮

```
> 帮我看下 src/main/java/com/miniclaudecode/agent/Agent.java 里的 run() 方法做了什么
```
...
1. **Lanterna 的线程安全模型**：Lanterna 所有 GUI 操作必须在事件线程（`screen.getEventThread()`）里执行。Agent 流式输出在后台线程，需要
   `postRunnable()` 封送到事件线程，否则会抛 `IllegalStateException` 或渲染撕裂。
2. **`TextArea` 大文本性能**：对话积累到 1000+ 条消息时，`TextArea` 会卡顿。解决方案：`TextArea` 只保留最近 200 条消息的渲染，历史对话按
   `ConversationSnapshot` 归档到 `ConversationStore`，`Ctrl+P` 恢复。
3. **代码高亮性能**：大文件（> 500 行）正则着色可能阻塞事件线程。解决：在后台线程做 `CodeHighlighter.highlight()`，完成后再
   `postRunnable` 更新 UI。
4. **JLine History 迁移**：现有 CLI 的 JLine `History` 是输入命令历史（`/plan`、`/search` 等），TUI 的
   `ConversationHistoryManager` 是 **对话快照历史**（用户和 Agent 的完整对话），两者不能混用。TUI 模式下 JLine History
   仍可保留（按 `↑` 在输入框内展示命令历史）。
5. **Lanterna 在 Windows ConEmu / Cmder 的兼容性**：部分 Windows 虚拟终端对 ANSI TrueColor 支持不完整，可能导致颜色显示异常。需要在
   Windows 上做 fallback 到 16 色方案。
6. **文件树的 `Files.walk` 性能**：大仓库（如 `mini-claude-code` 自己，src/ 下 100+ 文件）`Files.walk` 可能
   100ms+。解决：只懒加载一层（展开目录时 `Files.list`），不做预扫描。
7. **`CodeChunker` 的语言识别精度**：`CodeChunker` 靠文件名后缀识别语言（`.java` → `java`），没有后缀的文件（如 `Makefile`、
   `Dockerfile`）拿不到语言。解决：内置 `Makefile` / `Dockerfile` / `Jenkinsfile` 等无后缀文件的扩展识别表。
8. **Lanterna Screen 重绘频率**：流式输出每来一个 chunk 就 `postRunnable` 刷新 `TextArea`，事件线程会密集刷新屏幕。解决：做
   50ms 节流（`ScheduledExecutorService.scheduleAtFixedRate`），每 50ms 批量合并多个 chunk 一次性刷新。
9. **TUI 下的多行输入**：Lanterna `TextField` 默认是单行的。`Shift+Enter` 插入 `\n` 让输入框支持多行，提交时合并为一段。
10. **降级 CLI 模式的 TUI 专属功能降级行为**：`/config` 在 CLI 模式下应该降级为打印配置值（
    `MiniClaudeCodeConfig.toString()`），不弹出配置面板（因为 CLI 没有面板能力）。
...
| TUI vs CLI 关系        | **CLI 为主，TUI opt-in**（`MINI_CLAUDE_CODE_TUI=true` 或 `-Dmini-claude-code.tui=true` 才启动全屏界面） |
| 文件树 Git 状态        | **不做**（留后续期）                                                                                    |
...
| 主题持久化路径         | `~/.mini-claude-code/config.json` 的 `theme` 字段                                                       |
| 对话历史持久化         | `~/.mini-claude-code/history/conversations-YYYY-MM-DD.jsonl`（按天，JSONL）                             |
...
- `TuiBootstrapTest`：未显式设置 `MINI_CLAUDE_CODE_TUI=true` → `shouldUseTui()` 返回 `false`
- `TuiBootstrapTest`：`MINI_CLAUDE_CODE_TUI=true` 且 `NO_TUI=true` → `shouldUseTui()` 返回 `false`
...
- `Agent.java` / `PlanExecuteAgent.java` / `SubAgent.java`：流式输出时根据 `LanternaWindow.isTuiMode()` 分流到
  `ChatPane.append*()` 或 `System.out.print`
...
- **安装包分发**（参见 §6.7）：
    - `mvn clean package` 产出 `target/mini-claude-code-1.0-SNAPSHOT.jar`（pom.xml 仍保持 `1.0-SNAPSHOT`，Banner 显示
      `16.0.0`）
    - 配置 `maven-assembly-plugin` 或 `maven-shade-plugin` 做 **可执行 fat jar**（包含 Lanterna 依赖，用户 `java -jar`
      即可运行）
...
- [x] `com.miniclaudecode.tui` 包落地，`Main.java` 接入 TUI / CLI 分支
- [x] `shouldUseTui()` 默认返回 CLI；显式 `MINI_CLAUDE_CODE_TUI=true` / `-Dmini-claude-code.tui=true` 才检查 TUI 条件；
  `NO_TUI=true` 和小终端降级
- [x] 三栏布局渲染：文件树 + 对话流 + 状态栏 + 底部输入栏
- [x] TUI 输入桥接真实 Agent runtime：普通输入走 ReAct，`/plan <任务>` 走 Plan-and-Execute，`/team <任务>` 走 Multi-Agent
- [x] TUI 支持核心命令：`/clear`、`/context`、`/memory`、`/memory clear`、`/save <事实>`、`/hitl`、`/hitl on`、`/hitl off`、
  `/config`、`/cancel`、`/exit`
...
- [x] 对话历史写入 `~/.mini-claude-code/history/session_*.jsonl`
...
当前取舍：第 16 期先交付可运行 TUI 主链路与安全审批闭环；主题市场、完整历史恢复 UI、代码块折叠状态机、安装包 Release workflow
等保留为后续产品化增强，不作为本期阻塞项。
...
| TUI vs CLI                           | **CLI 为主，TUI opt-in**（`MINI_CLAUDE_CODE_TUI=true` 或 `-Dmini-claude-code.tui=true`） |
| 文件树 Git 状态                      | **不做**（留后续期）                                                                     |
| TUI 内嵌编辑器                       | **不做**                                                                                 |
| 多窗口 / 分屏                        | **本期不做**                                                                             |
| 主题                                 | **深色 / 浅色两种**，Catppuccin 调色板，`config.json` 持久化                             |
| 对话历史                             | **新设计 `ConversationSnapshot`**，`~/.mini-claude-code/history/` JSONL 持久化           |
...
| 文件树忽略规则                       | `~/.mini-claude-code/filetree-ignore.txt`（每行一个 glob），不存在用内置默认             |
...
- `~/.mini-claude-code/config.json` 已存在（第 15 期已有）
- `MINI_CLAUDE_CODE_TUI=true`
- 终端尺寸 ≥ 80×24
...
```
> 帮我看下 src/main/java/com/miniclaudecode/agent/Agent.java 里的 Agent 类主要做了什么
```
...
1. 未显式设置 `MINI_CLAUDE_CODE_TUI=true` → `shouldUseTui()` 返回 `false`
2. 不弹 Lanterna 全屏窗口
