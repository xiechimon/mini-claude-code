...
1. 准备测试图片，建议放在 `~/mini-claude-code-image-cases/` 下：
    - `shot.png`：正常 PNG，<100KB
    - `large.png`：体积 >5MB（可用 `dd if=/dev/urandom of=large.png bs=1M count=6`，再手动覆盖前几个字节让 MIME 探测出
      `image/png`，或直接拿一张高分辨率截图）
    - `not-image.txt`：纯文本文件
...
预期：CLI 不出现“未抓取剪贴板图片 / 请切换模型”一类拦截提示；请求仍按 `text + image_url/base64` 图片输入结构发送。若
provider 拒绝当前请求，应显示 provider 返回的真实错误。
...
- 自动写入 `~/.mini-claude-code/cache/clip-<ts>.png`
- note 行 `[已附加图片: 剪贴板 (clip-<ts>.png), mimeType=image/png, bytes=…]`
- LLM 回复明显基于图像内容

### Case 18 — Ctrl+V 注入

光标停在 Mini Claude Code 输入框，先复制好图（同 Case 17），然后按 **Ctrl+V**（不是 Cmd+V，Cmd+V 会被终端拦截成本地粘贴文本）。
...
通过 `ssh user@host` 进入远程主机，启动 Mini Claude Code，然后：
