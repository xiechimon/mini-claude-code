...
- 每个顶层类型都要添加简洁易懂的中文 Javadoc，内容聚焦职责、边界或设计原因。
- 生产代码的每个构造器和方法都要添加中文 Javadoc；接口方法必须完整描述契约。
- 每个类型参数和形参使用 `@param`，非 `void` 返回值使用 `@return`，声明的异常使用 `@throws`。
- 方法体注释用于解释命名和结构无法表达的设计原因、边界或副作用；优先通过命名和拆分方法表达意图。
- 私有辅助方法、字段和常量仅在存在不直观的约束、单位或副作用时补充注释。
- 测试类添加职责 Javadoc；测试方法优先使用行为式命名，仅在命名无法表达背景或原因时补充注释。
...
**上游仓库的锁定提交是唯一功能基准**，当前锁定 `eb4307f4e495d2ed22699e1e5682eb55f8076ade`（2026-08-12，PR #512
`feat/course-v22-refresh`）。每课开始时重新确认该提交是否仍是 `main` 最新，变更需评估影响面后再决定是否升级锁定。

官网 <https://learn.shareai.run/zh> **仅作参考，不作基准**。其页面数据来自构建期产物 `web/src/data/generated/`，实测存在
源码已改而该产物未重新生成的情况（提交 `4d8d420` 改了 s03 源码但未重新生成，导致官网 s02 与 s03 分别来自两个不同提交），
且部署可能落后于 `main`。

### 每课的循环

1. **课程基线** —— 完整阅读锁定提交下当前课的 `code.py`、`README.zh.md` 与站点数据（`scenarios/` `annotations/`
   `execution-flows.ts`），提取完整功能清单和可验证的验收标准。`code.py` 决定「必须有什么」，讲解与站点数据只在与源码
   一致时可信。
...
5. **对照复盘** —— 逐项记录 Python 与 Java 的行为映射、设计差异及理由。
6. **沉淀** —— 复盘结论写入该课票文件的 `## Comments`，必要时再提炼领域术语或 ADR。

### 功能对等约束 ⛔

- 锁定提交下当前课的 `code.py` 是功能范围和行为的基准，课程已有功能必须全部保留。
