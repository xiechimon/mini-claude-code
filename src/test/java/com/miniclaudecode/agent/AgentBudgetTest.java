...
class AgentBudgetTest {
    ...
    void systemPropertyCanStillOverrideDynamicTokenBudget() {
        String old = System.getProperty("mini-claude-code.react.token.budget");
        try {
            System.setProperty("mini-claude-code.react.token.budget", "12345");
            AgentBudget budget = AgentBudget.fromLlmClient(new GLMClient("test-key"));

            assertEquals(12345, budget.tokenBudget());
        } finally {
            if (old == null) {
                System.clearProperty("mini-claude-code.react.token.budget");
            } else {
                System.setProperty("mini-claude-code.react.token.budget", old);
            }
        }
    }
