...
class LongTermMemoryTest {
    ...
    void shouldStoreAndRetrieve() {
        ...
        assertEquals("项目使用Java 17", memory.retrieve("fact-1").get().content());
    }
    ...
    void shouldPreserveTimestampAfterReload() {
        ...
        LongTermMemory reloaded = new LongTermMemory(tempDir.toFile());
        assertEquals(timestamp, reloaded.retrieve("f1").orElseThrow().timestamp());
    }
    ...
    void shouldFilterProjectScopedMemories() {
        ...
        assertTrue(visible.stream().anyMatch(entry -> entry.id().equals("global")));
        assertTrue(visible.stream().anyMatch(entry -> entry.id().equals("project-a")));
        assertTrue(visible.stream().noneMatch(entry -> entry.id().equals("project-b")));
    }
