...
class McpClientTest {
    ...
    void initializeTimeoutCanBeOverriddenBySystemProperty() {
        String previous = System.getProperty("mini-claude-code.mcp.initialize.timeout.seconds");
        try {
            System.setProperty("mini-claude-code.mcp.initialize.timeout.seconds", "17");
            assertEquals(17, McpClient.initializeTimeoutSeconds());
        } finally {
            if (previous == null) {
                System.clearProperty("mini-claude-code.mcp.initialize.timeout.seconds");
            } else {
                System.setProperty("mini-claude-code.mcp.initialize.timeout.seconds", previous);
            }
        }
    }
    ...
    void closeIsBestEffortAndDoesNotBlockOnServerSilence() throws Exception {
        // close 不发 shutdown notification，避免 server 卡死时阻塞退出
