...
class RuntimeApiServerTest {
    ...
    @Test
    void acceptsMiniClaudeCodeApiKeyHeader(@TempDir Path tempDir) throws Exception {
        try (RuntimeThreadStore store = new RuntimeThreadStore(tempDir.resolve("runtime.db"));
             RuntimeApiServer server = new RuntimeApiServer(store, prompt -> "x", 0, "secret")) {
            server.start();
            HttpRequest request = HttpRequest.newBuilder(
                            URI.create("http://127.0.0.1:" + server.port() + "/v1/threads"))
                    .POST(HttpRequest.BodyPublishers.noBody())
                    .timeout(Duration.ofSeconds(3))
                    .header("X-Mini-Claude-Code-API-Key", "secret")
                    .build();
