...
3. **过程校验**：每一步的结果都是证据。失败有失败的信息（404、登录页、空正文、防爬提示），用证据更新判断，
   **不要在同一条工具上反复重试**。
...
- `close_page` 只能关 Mini Claude Code 自己 `new_page` 出来的 tab，不要尝试关用户原有的 Gmail / Slack。
- 浏览器读页面优先 `take_snapshot`（结构化 DOM 文本）而非 `take_screenshot`。截图只在用户明确要看视觉布局、颜色、遮挡、截图验收时使用；vision
  模型会收到图片，非 vision 模型仍只能走 fallback 文案。
...
返回干净 markdown，免登录。 **约束**：免费配额 20 RPM，不要批量刷；命中防爬严重的站点（小红书 / 微信文章详情）也会失败，最后还是回到浏览器
MCP。
...
```bash
ls ~/.mini-claude-code/skills-cache/web-access/references/site-patterns/
```
...
**写回机制**：操作中如果发现新站点的陷阱或有效模式，主动写到
`~/.mini-claude-code/skills/web-access/references/site-patterns/<domain>.md`（用户级目录，不要写 jar 内置缓存），格式参考已有文件。
...
任务包含多个 **独立**调研目标时（如同时调研 3 个公众号），可以拆给 sub-agent 并行：让 PlanExecuteAgent 把任务拆成 DAG
平行批次，或用 Multi-Agent 模式（`/team`）分给多个 worker。 **注意**：浏览器实例只有一个，浏览器操作仍要串行；只有
`web_fetch` / `web_search` 这种纯 HTTP 调用才能真并行。
...
- 不要替用户输用户名密码：Mini Claude Code 不做自动登录
- 不要把 references/ 写回 jar 缓存目录：写到 `~/.mini-claude-code/skills/web-access/references/site-patterns/` 用户级目录
