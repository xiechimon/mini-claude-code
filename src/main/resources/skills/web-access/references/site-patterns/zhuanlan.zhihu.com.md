...
1. **被风控时返回登录墙**：`web_fetch` 返回的 HTML 包含 `<title>知乎，让每一次点击都充满意义</title>` + 没有正文 →
   被风控。换浏览器 isolated 也只是延缓；真要稳定抓建议走 `r.jina.ai`。
2. **图片站外防盗链**：知乎图片域名 `pic1.zhimg.com` 等加了 Referer 校验。要把图保存下来必须带 `
