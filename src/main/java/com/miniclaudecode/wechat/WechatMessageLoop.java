...
/**
 * 长轮询 iLink 更新并将文本消息串行交给一个 Agent 会话
 * 同步游标会持久化，重复消息去重，会话过期时停止当前循环
 */
public class WechatMessageLoop {
    ...
    public void run() {
        if (account == null) {
            throw new IllegalStateException("未找到微信账号，请先执行 mini-claude-code wechat setup");
        }
        try {
    ...
    private void handleBypass(WechatCommandParser.Command command, String contextToken) throws IOException {
        switch (command.type()) {
            ...
            default ->
                    queue.add(new WechatMessage("", account.boundUserId(), contextToken, "/" + command.type().name().toLowerCase(), List.of()));
        }
    }
    ...
    private void drainQueue() throws IOException {
        ...
        switch (command.type()) {
            ...
            case MODEL -> {
                send("`/model` 微信侧切换将在模型配置命令抽取后启用；当前使用 Mini Claude Code 默认模型。");
                drainQueue();
                return;
            }
            case UNKNOWN -> {
