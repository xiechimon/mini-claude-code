...
/**
 * 通过子进程 stdin/stdout 传输逐行 JSON-RPC 消息
 * stderr 单独消费，避免服务端日志阻塞协议流
 */
    ...
    private void startStdoutReader() {
        Thread thread = new Thread(() -> {
            try (BufferedReader reader = new BufferedReader(
                ...
                appendStderr("[mini-claude-code] stdout reader stopped: " + e.getMessage());
            }
        }, "mini-claude-code-mcp-stdio-stdout");
    ...
    private void startStderrReader() {
        Thread thread = new Thread(() -> {
            try (BufferedReader reader = new BufferedReader(
                ...
                appendStderr("[mini-claude-code] stderr reader stopped: " + e.getMessage());
            }
        }, "mini-claude-code-mcp-stdio-stderr");
        thread.setDaemon(true);
        thread.start();
    }
