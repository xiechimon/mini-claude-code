package com.miniclaudecode.runtime.api;

import com.fasterxml.jackson.databind.JsonNode;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.miniclaudecode.runtime.task.TaskRunner;
...
/**
 * 仅绑定 loopback 的轻量 Runtime HTTP API
 *
 * <p>所有 endpoint 都要求 Bearer 或兼容 API-key header
 * turn 以 {@code 202} 异步执行，结果写入持久事件流
 * SSE 的 {@code after} 游标只返回 {@code id > after} 的有序事件
 * 后台失败记录为 {@code turn.failed} 事件
 */
public class RuntimeApiServer implements AutoCloseable {
    ...
    private final ExecutorService executor = Executors.newCachedThreadPool(r -> {
        Thread thread = new Thread(r, "mini-claude-code-runtime-api");
    ...
    public RuntimeApiServer(RuntimeThreadStore store, TaskRunner runner, int port, String apiKey) throws IOException {
        if (apiKey == null || apiKey.isBlank()) {
            throw new IllegalArgumentException("Runtime API 需要配置 MINI_CLAUDE_CODE_RUNTIME_API_KEY 或 -Dmini-claude-code.runtime.api.key");
        }
        this.store = store;
        this.runner = runner;
        this.apiKey = apiKey;
