...
/**
 * 用 SQLite 持久化 runtime thread 及其有序事件
 * 所有操作串行使用同一连接，事件查询只返回 id 大于 after 的记录
 */
public class RuntimeThreadStore implements AutoCloseable {
    ...
    public static Path defaultDbPath() {
        String configured = System.getProperty("mini-claude-code.runtime.dir");
        if (configured == null || configured.isBlank()) {
            configured = System.getenv("MINI_CLAUDE_CODE_RUNTIME_DIR");
        }
        if (configured == null || configured.isBlank()) {
            configured = Path.of(System.getProperty("user.home"), ".mini-claude-code", "runtime").toString();
        }
