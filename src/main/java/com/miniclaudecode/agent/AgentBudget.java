 ...
 * 1. 系统属性：{@code mini-claude-code.react.token.budget} / {@code mini-claude-code.react.stagnation.window} /
 * {@code mini-claude-code.react.hard.max.iterations}
 ...
 * 场景（CI / 自动化批跑）通过 {@code -Dmini-claude-code.react.token.budget=N} 显式启用
...
public class AgentBudget {
    ...
    public static AgentBudget fromLlmClient(LlmClient llmClient) {
        ...
        // 显式 -Dmini-claude-code.react.token.budget=N 仍可启用硬预算，覆盖默认
        return new AgentBudget(
                readIntProperty("mini-claude-code.react.token.budget", Integer.MAX_VALUE),
