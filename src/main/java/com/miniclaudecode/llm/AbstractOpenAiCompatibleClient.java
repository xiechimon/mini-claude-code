...
/**
 * 复用 OpenAI-compatible Chat Completions 的请求、SSE 和工具调用协议
 * 子类只提供 provider 差异，不应绕过统一消息序列化和流式收尾语义
 */
public abstract class AbstractOpenAiCompatibleClient implements LlmClient {
    ...
    protected static final OkHttpClient SHARED_HTTP_CLIENT = new OkHttpClient.Builder()
            .connectTimeout(readTimeoutSeconds("mini-claude-code.llm.connect.timeout.seconds", 60), TimeUnit.SECONDS)
            .readTimeout(readTimeoutSeconds("mini-claude-code.llm.read.timeout.seconds", 300), TimeUnit.SECONDS)
            .writeTimeout(readTimeoutSeconds("mini-claude-code.llm.write.timeout.seconds", 60), TimeUnit.SECONDS)
            .callTimeout(readTimeoutSeconds("mini-claude-code.llm.call.timeout.seconds", 600), TimeUnit.SECONDS)
            .build();
