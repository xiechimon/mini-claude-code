...
/**
 * 按配置选择首个可用 provider 并创建对应 LlmClient
 */
public class LlmClientFactory {
    ...
    public static LlmClient create(String provider, MiniClaudeCodeConfig config) {
    ...
    public static LlmClient createFromConfig(MiniClaudeCodeConfig config) {
        LlmClient client = create(config.getDefaultProvider(), config);
        if (client != null) {
            return client;
        }

        for (String provider : new String[]{"glm", "deepseek", "kimi", "freellmapi"}) {
