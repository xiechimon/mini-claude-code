...
public record SnapshotConfig(
    ...
    public static SnapshotConfig fromEnvironment() {
        boolean enabled = readBoolean("mini-claude-code.snapshot.enabled", "MINI_CLAUDE_CODE_SNAPSHOT_ENABLED", true);
        Path root = Path.of(readString("mini-claude-code.snapshot.dir", "MINI_CLAUDE_CODE_SNAPSHOT_DIR",
                Path.of(System.getProperty("user.home"), ".mini-claude-code", "snapshots").toString()));
        int max = readInt("mini-claude-code.snapshot.max", "MINI_CLAUDE_CODE_SNAPSHOT_MAX", 50);
        List<String> excludes = mergeExcludes(readString("mini-claude-code.snapshot.excludes", "MINI_CLAUDE_CODE_SNAPSHOT_EXCLUDES", ""));
