...
/**
 * 使用 ripgrep JSON 输出执行代码搜索
 * 命令不可用、超时或解析失败时自动回退 JavaCodeSearchEngine
 */
final class RipgrepCodeSearchEngine implements CodeSearchEngine {
    ...
    public CodeSearchResult search(CodeSearchRequest request) {
        if (Boolean.getBoolean("mini-claude-code.search.disable.rg") || !isRipgrepAvailable()) {
        ...
        try {
            ...
            readerExecutor = Executors.newSingleThreadExecutor(r -> {
                Thread thread = new Thread(r, "mini-claude-code-rg-reader");
