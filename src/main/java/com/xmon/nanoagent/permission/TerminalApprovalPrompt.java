 ...
 * 由 {@code Repl} 包住整个回合的退出分支接住，整个会话随之结束。这与课程行为一致，区别只在于此处的
 * 异常是有类型的。注意这条依赖 {@code Repl} 的 try 覆盖 {@code respond} 调用——只包住主提示符是接不住的。
 */
public final class TerminalApprovalPrompt implements ApprovalPrompt {
     ...
     * @param output 命令行输出
     */
    public TerminalApprovalPrompt(LineReader input, PrintWriter output) {
