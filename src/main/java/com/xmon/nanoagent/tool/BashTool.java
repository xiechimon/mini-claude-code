 ...
 * 执行 Bash 命令
 *
 * <p>不受 {@link Workspace} 约束，自身也不做任何命令拦截：危险命令由 {@code PermissionGate} 在工具执行
 * 之前统一裁决。把拦截留在工具内部会让策略随工具数量重复，新增工具时也容易整条漏掉。
 */
public final class BashTool implements ToolHandler {
     ...
     * @return 命令工具
     */
    public static BashTool production(Path workingDirectory, Map<String, String> environment) {
     ...
     */
    public BashTool(Path workingDirectory, Map<String, String> environment, Duration timeout) {
